...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const signUp = async (email: string, password: string) => {
    ...
    // needsConfirmation = signup succeeded but no session yet (email confirmation required)
    const needsConfirmation = !error && !data.session;
    return { error: error?.message ?? null, needsConfirmation, userId: data.user?.id ?? null };
  };
